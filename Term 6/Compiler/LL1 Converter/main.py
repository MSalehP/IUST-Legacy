...
def recursionHandle(k, next_i, grammar_follow, i, grammar, start, grammar_first, lhs):
    if len(k) == next_i:
        ...
        if k[next_i].isupper():
            for q in grammar_first[k[next_i]]:
                if q == "`":
                    grammar_follow = recursionHandle(
                        k,
                        next_i + 1,
                        grammar_follow,
                        i,
    ...
    for i in grammar:
        ...
        for k in j:
            if lhs in k:
                ...
                grammar_follow = recursionHandle(
                    k, next_i, grammar_follow, i, grammar, start, grammar_first, lhs
                )
    if lhs == start:
...
def generate_parse_table(
    terminals, non_terminals, grammar, grammar_first, grammar_follow
):
    ...
    for non_terminal in non_terminals:
        for terminal in terminals:
            ...
            parse_table[non_terminals.index(non_terminal)][
...
def parse(expr, parse_table, terminals, non_terminals):
    ...
    while True:
        ...
        if stack[0] == expr[0] and stack[0] == "$":
            ...
            action = parse_table[non_terminals.index(stack[0])][
                terminals.index(expr[0])
            ]
            stack.pop(0)
            i = 0
